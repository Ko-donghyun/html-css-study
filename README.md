...
- CSS
  ...
  - !!주의!!
    - 직접 스타일을 주지 말 것!
    - 스타일 정보는 웹 문서를 브라우저 화면에 표시하기 전에 결정되어야 하기 때문에, 모든 스타일 정보는 <head> 태그와 </head> 태그 안에서 정의해야 한다.
      - 따라서 위에서 먼서 스타일을 설정해 놓더라도 이후에 스타일을 설정하게 되면 이후에 설정한 스타일이 적용됨
    - 태그 < 클래스 < ID - 주로 클래스로 처리.
    - 스타일은 상속된다.
      - 웹 문서에서 사용하는 여러 태그들은 서로 포함 관계를 가지고 있어 포함하는 태그는 부모 요소라고 하고, 포함된 태그는 자식 요소라고 합니다.
      - 스타일시트에서는 부모 요소에 있는 속성들이 자식 요소로 전달된다.
    - 스타일은 단계적으로 적용된다.
      - 부모 요소로부터 자식 요소로 단계별로 스타일이 적용됨.
    - CSS3는 표준 규약이 완성되어 있지 않아 브라우저들이 새로운 CSS3 속성을 부분적으로만 구현 가능. 따라서 CSS3의 속성을 사용하고자 한다면, 속성 이름 앞에 브라우저를 식별할 수 있도록 접두사를 붙여야 한다.
      - `-webkit-transform: translate(-50%, -50%);` 사파리와 크롬 브라우저용 속성
      - `-moz-transform: translate(-50%, -50%);` 파이어폭스 브라우저용 속성
      - `-ms-transform: translate(-50%, -50%);` 인터넷 익스플로러
      - `-o-transform: translate(-50%, -50%);` 오페라 브라우저
      - `transform: translate(-50%, -50%);` 표준이 정해진 뒤 사용하게 될 속성
